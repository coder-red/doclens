...
![DocLens demo: upload → extraction → validation → approved vs flagged routing](docs/demo.gif)
...
PDF/img/docx          (rasterize)        (JSON out,   13 rules   ├─ approved ▶ Excel + webhook
                      ...
                      Pillow             retry if     checks     └─ failed   ▶ audit log only
                                         off-schema)             (everything lands in SQLite)
```
